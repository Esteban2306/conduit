...
export class MessageWorker implements OnModuleInit, OnModuleDestroy {
  ...
  private async processJob(job: Job<MessageJobPayload>) {
    const {
      messageId,
      channel,
      recipient,
    ...
    await this.updateMesageStatus(messageId, MessageStatus.PROCESSING);
    ...
    try {
      ...
      const maxAttempts = this.config.get<number>('queue.maxAttemps') ?? 3;
      const isLastAttemps = job.attemptsMade + 1 >= maxAttempts;

      if (isLastAttemps) {
        await this.updateMesageStatus(
          messageId,
          MessageStatus.DEAD,
          error.message,
        );
  ...
  private async updateMesageStatus(
    messageId: string,
